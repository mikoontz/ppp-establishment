# This script generates figure depicting establishment probability for microcosm and simulations at absolute generation 9 in one panel and 5 generations after final introduction in the second panel.
...
## Absolute time first
# Simulation data from generation 9
sims_establish_absolute <- sims_results %>%
  ...
  select(propagule_number, env, t_equals_9) %>% # Here is where we get the simulation results from GEN 9
  group_by(propagule_number) %>%
  summarize(establish_prop = mean(t_equals_9)) %>% # Here is where we get the simulation results from GEN 9
  as.data.frame()
...
# Using establishment assessment at generation 9
prob_establishment_model <- glmer(extant9 ~ number + environment + number:environment + (1 | block), data=b.trim, family=binomial, control=glmerControl(optimizer="bobyqa")) 
...
pdf("figures/establishment-probability-absolute-9-and-relative-5-time-experiment-and-simulations.pdf", height = 3.14961, width = 2 * 3.14961)
# postscript("figures/establishment-probability-absolute-9-and-relative-5-time-experiment-and-simulations.eps", height = 3.14961, width = 2 * 3.14961)
...
# Simulation data from 5 generations after final introduction event:

# Can't average across environment treatments anymore
